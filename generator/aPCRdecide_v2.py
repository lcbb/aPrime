...
def primer_tm(temp, length): # inputs: temp = template sequence, length = desired length amplified sequence
	...
	for p in range(18,23): # for first primer length [18,23)
		for q in range(0, int(len(temp)/2)): # for starting positions of the amplified length from 0 to end of template (template input sequence is duplicated to help with rollover)
...
#
fInM13for=open("./m13.txt","r")
fInM13rev=open("./m13_rev.txt", "r")
tmplA=[]
tmpSeq=''
for line in fInM13for:
	tmpSeq+=line.rstrip().lstrip()
tmplA.append(tmpSeq);
tmpSeq=''
for line in fInM13rev:
	tmpSeq+=line.rstrip().lstrip()
tmplA.append(tmpSeq);
fInM13for.close();
fInM13rev.close();
...
for i in range(500, 7000):
	...
	for templDNA in tmplA:
		...
		for j in range(len(primers[0])):
			...
			exState="INSERT INTO primers VALUES ('"+tmpUID+"',"+str(i)+",'"+primers[0][j]+"','"+primers[1][j]+"',"+str(primers[2][j])+","+str(primers[3][j])+",'M13','','')"
#			print(exState);
			c.execute(exState)
...
conn.commit();
c.close()
#	for templDNA in tmplB:
#		primers=primer_tm(templDNA, targetLen);
#		for j in range(len(primers[0])):
#			tmpUID=str(md5.new(primers[0][j]+primers[1][j]).hexdigest())
#			exState="INSERT INTO primers VALUES ('"+tmpUID+","+str(i)+",'"+primers[0][j]+"','"+primers[1][j]+"',"+str(primers[2][j])+","+str(primers[3][j])+"'Lambda','','')"
#			print(exState);
